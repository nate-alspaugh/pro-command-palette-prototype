...
const componentsList: ComponentDefinition[] = [
  ...
  },
  {
    id: 'button-primary',
    name: 'Button Primary',
    description: 'High-fidelity primary button with gradient background and dual borders.',
    category: 'Elements',
    properties: [
  ...
  },
  {
    ...
    description: 'Glass/ghost secondary button with subtle gradient and borders.',
    category: 'Elements',
    properties: [
  ...
  },
  {
    ...
    description: 'Individual analyst predictor card showing rank, accuracy, and performance metrics.',
    category: 'Cards',
    properties: [
  ...
  },
  {
    ...
    description: 'Horizontal bar chart for comparing values with inline labels. Colors bars automatically based on value ratio (green ≥75%, yellow 65-74%, red <65%).',
    category: 'Charts',
    properties: [
      { id: 'showLabels', name: 'Show Labels', type: 'boolean', default: true },
  ...
  },
  {
    id: 'area-chart',
    name: 'Area Chart',
    description: 'Area chart for showing cumulative trends with filled regions.',
    category: 'Charts',
    properties: [
      { id: 'color', name: 'Color', type: 'select', options: ['accent-green', 'accent-blue', 'accent-red', 'accent-yellow'], default: 'accent-green' },
      { id: 'showLabels', name: 'Show Labels', type: 'boolean', default: true }
  ...
  },
  {
    id: 'donut-chart',
    name: 'Donut Chart',
    description: 'Circular donut chart (pie chart style) for displaying multiple data segments with different values and colors. Supports up to 8 segments.',
    category: 'Charts',
    properties: [
  ...
  },
  {
    id: 'tab',
    name: 'Tab',
    description: 'Individual navigation tab with active/inactive states and parallax glow effect.',
    category: 'Navigation',
    properties: [
  ...
  },
  {
    id: 'tab-rail',
    name: 'Tab Rail',
    description: 'Horizontal rail of tabs with parallax-glow hover effects.',
    category: 'Navigation',
    properties: [],
    render: () => <NavTabs />,
...
const ComponentLibrary = ({ initialComponentId, onNavigate }: ComponentLibraryProps) => {
  ...
  const [customDescriptions, setCustomDescriptions] = useState<Record<string, string>>(() => {
    const saved = localStorage.getItem(STORAGE_KEYS.COMPONENT_DESCRIPTIONS)
    return saved ? JSON.parse(saved) : {}
  })
  const [isEditingDescription, setIsEditingDescription] = useState(false)
  const [editedDescription, setEditedDescription] = useState('')

  const getDescription = (component: ComponentDefinition): string => {
    return customDescriptions[component.id] || component.description
  }

  const handleSaveDescription = (componentId: string): void => {
    const newDescriptions = { ...customDescriptions, [componentId]: editedDescription }
    setCustomDescriptions(newDescriptions)
    localStorage.setItem(STORAGE_KEYS.COMPONENT_DESCRIPTIONS, JSON.stringify(newDescriptions))
    setIsEditingDescription(false)
  }

  const handleResetDescription = (componentId: string): void => {
    const newDescriptions = { ...customDescriptions }
    delete newDescriptions[componentId]
    setCustomDescriptions(newDescriptions)
    localStorage.setItem(STORAGE_KEYS.COMPONENT_DESCRIPTIONS, JSON.stringify(newDescriptions))
    setIsEditingDescription(false)
  }
              ...
              <div className="panel-header" style={{ marginTop: '32px' }}>
                ...
                {!isEditingDescription && (
                  <button 
                    ...
                    onClick={() => {
                      setEditedDescription(getDescription(selectedComponent))
              ...
              {isEditingDescription ? (
                <div className="description-edit-container">
                  ...
                  <div className="description-edit-actions">
                    <button 
                      className="btn-save"
                      onClick={() => handleSaveDescription(selectedComponent.id)}
                    >
                      Save
                    </button>
                    <button 
                    ...
                    {customDescriptions[selectedComponent.id] && (
                      <button 
                        className="btn-reset"
                        onClick={() => handleResetDescription(selectedComponent.id)}
                      >
                        Reset
                      </button>
                    )}
                ...
                <p className="prop-description">{getDescription(selectedComponent)}</p>
              )}
            </div>
          )}
        </div>
